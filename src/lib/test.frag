...
float makeLine(float val) {
  return step(u_radius, 1.0 - abs(val - 0.5));
}
...
void main() {
  ...
  // vec3 res = mix(vec3(0), vec3(1), vUv.x);
  // vec2 uv = vUv;
  // uv -= vec2(0.5);
  // uv *= 2.0;
  //vec3(step(u_radius, length(uv)))
  //fract(vUv.x * 10.0)
  //step(1.0, mod(vUv.x * 10.0, 2.0)))
  //vec3(mix(0.0, 0.5, vUv.x))
