...
float fit(float unscaled, float ogMin, float ogMax, float minAllowed, float maxAllowed) {
  return (maxAllowed - minAllowed) * (unscaled * ogMax) / (ogMax - ogMin) + minAllowed;
}

uniform float u_time;

float wave(float i) {
  float y = i + u_time * .00009;

  float fracts = fract(y * 5.0);
  float stepped = smoothstep(0.01, 0.5, fracts);

  if (fracts >= 0.5)
    stepped = smoothstep(0.99, 0.5, fracts);

  return fit(stepped, 0.0, 1.0, 0.2, 0.8);
}
...
void main() {
  ...
  vec3 coords = normal + noise(normal);
  // vec3 coords = normal;
  
  vDisplacement = wave(coords.y);
